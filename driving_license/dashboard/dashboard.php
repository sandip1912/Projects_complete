<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
     <!-- LINKS -->
     <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.0/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-KyZXEAg3QhqLMpG8r+8fhAXLRk2vvoC2f3B09zVXn8CA5QIVfZOJ3BCsw2P0p/We" crossorigin="anonymous">
     <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.0/dist/js/bootstrap.bundle.min.js" integrity="sha384-U1DAWAznBHeqEIlVSCgzq+c9gqGAJn5c/t99JyeKa9xxaYpSvHU5awsuZVVFIhvj" crossorigin="anonymous"></script>
   <link rel="stylesheet" href="../css/style_dashboard.css">
    <title>Document</title>
</head>
<body>
    <!-------------------- heading -------------------->
     <section id="heading">
    <img src="../home/sarathi_logo12.png" alt="">
    </section>
    <!-------------------- menu -------------------->
   
    <section id="menu">
    <!-------------------- options-output -------------------->
    <section id="options">
        <ul>
            <li><a href="../home.php">HOME</a></li>
            <li><a href="../user/userLogin.php">USER LOGIN</a></li>
            <li><a href="../user/userregistration.php">USER REGISTRATION</a></li>
            
            <li><a href="../admin/adminlogin.php">ADMIN LOGIN</a></li>
            <li><a href="../services/applicationststus.php">APPLICATION STATUS</a></li>
            <li><a href="../services/printlicense.php">PRINT LICENSE</a></li>
        </ul>
    </section>
    <section id="output">
<!-------------------- output -------------------->
<h1 style="text-align: center;margin:5vh 0;color: #0586cc;">WE CAN HELP YOU WITH...</h1>

<img src="../home/bannerpic.jpg" alt="" srcset="">

<!-------------------- output -------------------->
    </section> 

    </section> 
</body>
</html>